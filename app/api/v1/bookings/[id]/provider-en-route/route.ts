...
/**
 * Spec 020 §3, `POST /api/v1/bookings/{id}/provider-en-route` — AC-4, AC-7, AC-11.
 *
 ...
 * `provider_en_route` returns `200` with the current booking, which is why this route needs no
...
export const POST = withApiRoute(async (request, correlationId) => {
  ...
  const booking = await advanceBooking(session.userId, profile.id, bookingIdFromUrl(request, 1), 'provider_en_route');
