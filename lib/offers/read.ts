...
export async function loadOfferDto(offerId: string, db: Executor = getDb()): Promise<OfferDto> {
  const [row] = await queryRows<OfferRow>(
    db,
    sql`SELECT ${OFFER_COLUMNS} FROM offers o JOIN provider_profiles pp ON pp.id = o.provider_profile_id
        WHERE o.id = ${offerId} AND o.status <> 'draft'`,
  );
 ...
 */
async function markViewed(customerUserId: string, scope: { requestId: string } | { offerId: string }): Promise<void> {
  const filter = 'requestId' in scope ? sql`request_id = ${scope.requestId}` : sql`id = ${scope.offerId}`;
...
async function assertCustomerOwnsRequest(userId: string, requestId: string): Promise<void> {
